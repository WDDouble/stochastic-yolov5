...
class model:
    ...
    def run(self):
        old_time = time.time()
        cfg_list=["yolov5s-dropout.yaml","yolov5s-gdropout.yaml","yolov5s-dropblock.yaml","yolov5s-dg.yaml","yolov5s-db.yaml","yolov5s-gd.yaml"]
        print("running yolov5...")
        subprocess.call(['python', 'val.py','--cfg',cfg_list[self.dropout_type],'--batch','16','--data','coco.yaml','--imgsz','640','--iou-thres','0.6','--num_samples','8','--conf-thres','0.5','--new_drop_rate',self.drop_rate,'--second_drop_rate',self.second_dropout_rate],stdout=subprocess.DEVNULL,stderr=subprocess.STDOUT)
        ...
        subprocess.call(['python', 'val.py','--cfg',cfg_list[self.dropout_type],'--batch','16','--data','coco.yaml','--imgsz','640','--iou-thres','0.6','--num_samples','8','--conf-thres','0.5','--new_drop_rate',self.drop_rate,'--corruption_num','7','--severity','3','--second_drop_rate',self.second_dropout_rate],stdout=subprocess.DEVNULL,stderr=subprocess.STDOUT)
        subprocess.call(['python', 'pdq_evaluation/evaluate.py','--test_set','coco','--gt_loc','/content/datasets/coco/annotations/instances_val2017.json','--det_loc','/content/stochastic-yolov5/dets_converted_exp_0.5_0.6.json','--save_folder','output','--num_workers','15'],stdout=subprocess.DEVNULL,stderr=subprocess.STDOUT)
        with open(r"./output/scores.txt") as f:
            for line in f.readlines():
                if line:
        ...
        subprocess.call(['python', 'val.py','--cfg',cfg_list[self.dropout_type],'--batch','16','--data','coco.yaml','--imgsz','640','--iou-thres','0.6','--num_samples','8','--conf-thres','0.5','--new_drop_rate',self.drop_rate,'--corruption_num','8','--severity','3','--second_drop_rate',self.second_dropout_rate],stdout=subprocess.DEVNULL,stderr=subprocess.STDOUT)
        subprocess.call(['python', 'pdq_evaluation/evaluate.py','--test_set','coco','--gt_loc','/content/datasets/coco/annotations/instances_val2017.json','--det_loc','/content/stochastic-yolov5/dets_converted_exp_0.5_0.6.json','--save_folder','output','--num_workers','15'],stdout=subprocess.DEVNULL,stderr=subprocess.STDOUT)
        with open(r"./output/scores.txt") as f:
            for line in f.readlines():
                if line:
        ...
        subprocess.call(['python', 'val.py','--cfg',cfg_list[self.dropout_type],'--batch','16','--data','coco.yaml','--imgsz','640','--iou-thres','0.6','--num_samples','8','--conf-thres','0.5','--new_drop_rate',self.drop_rate,'--corruption_num','9','--severity','3','--second_drop_rate',self.second_dropout_rate],stdout=subprocess.DEVNULL,stderr=subprocess.STDOUT)
        subprocess.call(['python', 'pdq_evaluation/evaluate.py','--test_set','coco','--gt_loc','/content/datasets/coco/annotations/instances_val2017.json','--det_loc','/content/stochastic-yolov5/dets_converted_exp_0.5_0.6.json','--save_folder','output','--num_workers','15'],stdout=subprocess.DEVNULL,stderr=subprocess.STDOUT)
        with open(r"./output/scores.txt") as f:
            for line in f.readlines():
                if line:
...
class MyProblem(ElementwiseProblem):
    def __init__(self):
        super().__init__(n_var=3, n_obj=4, n_constr=1, xl=np.array([0, 0, 0]), xu=[0.5,0.5,5])
